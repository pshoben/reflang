#include "serializer_yaml.class.hpp"
...
namespace
{
	...
	string GetFieldNames(const Class& c)
	{
		stringstream tmpl;
		tmpl << "	std::vector<std::string> ret;\n";
		for (const auto& field : c.Fields) {
			tmpl << "	ret.push_back(\"" << field.Name << "\");\n";
		}
		tmpl << "	return ret;\n";
		return tmpl.str();
	}
	...
	string GetStaticFieldNames(const Class& c)
	{
		stringstream tmpl;
		tmpl << "	std::vector<std::string> ret;\n";
		for (const auto& field : c.StaticFields) {
			tmpl << "	ret.push_back(\"" << c.GetFullName() << "::" << field.Name << "\");\n";
		}
		tmpl << "	return ret;\n";
		return tmpl.str();
	}
...
public:
	...
	template <typename T>
	static std::vector<std::string> GetFieldNames(const %name%& c, T t);

	template <typename T>
	static std::vector<std::string> GetFieldNames(%name%& c, T t);

	template <typename T>
	static void IterateStaticFields(T t);

	template <typename T>
	static std::vector<std::string> GetStaticFieldNames(const %name%& c, T t);
...
template <typename T>
std::vector<std::string> Class<%name%>::GetFieldNames(const %name%& c, T t)
{
%get_field_names%}

template <typename T>
std::vector<std::string> Class<%name%>::GetFieldNames(%name%& c, T t)
{
%get_field_names%}
...
template <typename T>
std::vector<std::string> Class<%name%>::GetStaticFieldNames(const %name%& c, T t)
{
...
%methods_decl%%static_methods_decl%
	...
	o << ReplaceAll(
			...
			{
				{"%name%", c.GetFullName()},
				{"%get_field_names%", GetFieldNames(c)},
				{"%get_static_field_names%", GetStaticFieldNames(c)},
